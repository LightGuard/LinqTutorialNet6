...
namespace Exercises
{
    public static class PrependAppend
    {
        ...
        {
            if (!words.Any()) return words;
            
            var newWords = words;
            if (!words.First().Equals("START")) newWords = newWords.Prepend("START");
            if (!words.Last().Equals("END")) newWords = newWords.Append("END");
