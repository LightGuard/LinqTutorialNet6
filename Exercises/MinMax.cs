...
namespace Exercises
{
    public static class MinMax
    {
        ...
        {
            return words.Any() ? words.Min(w => w.Count())  : null;
        }
        ...
        public static int CountOfLargestNumbers(IEnumerable<int> numbers)
        {
            ...
            var max =  numbers.Max();
            return numbers.Any() ? numbers.Count(n => n == max) : 0;
        }
        ...
        {
            return !owners.Any() ? 0 : owners.Max(owner => owner.Pets.Count(p => p.PetType == PetType.Dog));
        }
